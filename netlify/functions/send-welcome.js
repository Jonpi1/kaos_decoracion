...
<body style="margin:0;padding:0;background:#FAF7F2;font-family:'Georgia',serif;">
  ...
  const payload = JSON.stringify({
    from: { email: 'kaosdecoracion@gmail.com', name: 'Kaos Decoración' },
    to: [{ email, name }],
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        if (res.statusCode === 202) {
          ...
          resolve({ statusCode: 200, body: JSON.stringify({ ok: false, error: data }) });
        }
      });
    });
    req.on('error', (e) => {
      console.error('Request error:', e.message);
      resolve({ statusCode: 200, body: JSON.stringify({ ok: false }) });
    });
    req.write(payload);
    req.end();
  });
